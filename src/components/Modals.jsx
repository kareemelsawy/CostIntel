...
export function SKUDetailModal({ sku, materials, accessories, commercial, onClose, onEdit, onCalc, onSaveSku }) {
  ...
  const matOpts=materials.map(mt=>({value:mt.material_id,label:mt.name}))

  const tableRows=[]
  cost.materials_breakdown.forEach(mb=>{mb.panels.forEach((p,j)=>{tableRows.push({type:'panel',mb,p,j,isFirst:j===0,rowSpan:mb.panels.length})})})
  tableRows.push({type:'edge'})

  function handleApplyMaterials(){
    if(onSaveSku) onSaveSku({...sku,body_material_id:bodyMat,back_material_id:backMat,door_material_id:doorMat})
  }
  ...
  return (
    <Modal onClose={onClose} width={760}>
      ...
      <div style={{marginBottom:16}}>
        <div style={{...lSt(),marginBottom:10}}>Material & Edge Banding</div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            ...
            <tbody>
              {tableRows.map((row,idx)=>{
                if(row.type==='edge') return <tr key="edge" style={{borderBottom:`1px solid ${COLORS.border}`,background:COLORS.surfaceHover}}>
                  ...
                  <td style={{padding:'6px 12px',textAlign:'right',color:COLORS.textDim}}>—</td><td style={{padding:'6px 12px',textAlign:'right',color:COLORS.textDim}}>—</td>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(155px,1fr))',gap:8}}>
          {cost.accessories.items.map(a=>(<div key={a.acc_id||a.name} style={{background:COLORS.bg,borderRadius:8,padding:'10px 12px'}}><div style={{fontSize:11,color:COLORS.textMuted,marginBottom:2}}>{a.name} ({a.area_m2?`${a.area_m2.toFixed(2)} m²`:a.qty})</div><div style={{fontSize:14,fontWeight:700,color:COLORS.text}}>{fmt(a.cost)} EGP</div><div style={{fontSize:10,color:COLORS.textMuted}}>@ {fmt(a.unit_price)} each</div></div>))}
...
export function EditSKUModal({ sku, materials, catDefaults, onSave, onClose }) {
  const [f,setF]=useState({...sku}); const isNew=sku._isNew
  const set=(k,v)=>setF(p=>{const n={...p,[k]:v};if(k==='door_type')n.has_sliding_system=v==='Sliding';if(k==='sub_category'){const d=catDefaults[v]||catDefaults['Other'];n.body_material_id=d.body;n.back_material_id=d.back;n.door_material_id=d.door};return n})
