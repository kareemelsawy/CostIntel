...
export default function PricingPage({ materials, setMaterials, accessories, setAccessories, commercial, setCommercial, setEditingMat, toast }) {
  ...
  function handleMatImport(e){
    ...
    const reader=new FileReader();reader.onload=ev=>{
      try{const lines=ev.target.result.split('\n').filter(l=>l.trim());const hdrs=lines[0].split(',').map(h=>h.trim());let u=0
      for(let i=1;i<lines.length;i++){const vals=lines[i].split(',');const row={};hdrs.forEach((h,j)=>{row[h]=vals[j]?.trim()});const mid=row.material_id||row.ID;if(!mid)continue
      setMaterials(p=>p.map(m=>{if(m.material_id===mid){u++;return{...m,price:row.price?Number(row.price):m.price,price_good:row.price_good?Number(row.price_good):m.price_good,name:row.name||m.name}};return m}))}
      toast(`Updated ${u} material prices`)}catch(err){toast('Error: '+err.message,'error')}};reader.readAsText(file);e.target.value=''}

  function handleAccImport(e){
    const file=e.target.files?.[0];if(!file)return
    const reader=new FileReader();reader.onload=ev=>{
      try{const lines=ev.target.result.split('\n').filter(l=>l.trim());const hdrs=lines[0].split(',').map(h=>h.trim());let u=0
      for(let i=1;i<lines.length;i++){const vals=lines[i].split(',');const row={};hdrs.forEach((h,j)=>{row[h]=vals[j]?.trim()});const aid=row.acc_id||row.ID;if(!aid)continue
      setAccessories(p=>p.map(a=>{if(a.acc_id===aid){u++;return{...a,price:row.price?Number(row.price):a.price,price_good:row.price_good?Number(row.price_good):a.price_good}};return a}))}
      toast(`Updated ${u} accessory prices`)}catch(err){toast('Error: '+err.message,'error')}};reader.readAsText(file);e.target.value=''}
  ...
  return (
    <div style={{padding:'24px 28px',overflowY:'auto',flex:1}}>
      ...
      <p style={{fontSize:13,color:COLORS.textMuted,marginBottom:20}}>Edit inline or mass upload via CSV</p>
      ...
      <Card style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:12}}>
          {[{l:'VAT %',k:'vat_percent'},{l:'Commission %',k:'commission_percent'},{l:'Overhead %',k:'overhead_percent'},{l:'Seller Margin %',k:'seller_margin_percent'}].map(f=>(
