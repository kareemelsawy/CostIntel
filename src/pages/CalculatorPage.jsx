...
export default function CalculatorPage({ materials, accessories, commercial, setSkus, toast, prefill, clearPrefill, catDefaults }) {
  ...
  function Field({label,k,type='number',options,min=0}) {
    ...
    return <div><label style={lSt()}>{label}</label><input type="number" value={form[k]} onChange={e=>set(k,Number(e.target.value))} min={min} style={iSt()}/></div>
  }

  const matOpts = materials.map(m=>({value:m.material_id,label:`${m.name} (${fmt(useGood?(m.price_good||m.price):m.price)})`}))
  const hingeOpts = [{value:'',label:'Auto'},...accessories.filter(a=>a.group==='hinge').map(a=>({value:a.acc_id,label:`${a.name} (${useGood?(a.price_good||a.price):a.price})`}))]
  const handleOpts = [{value:'',label:'Auto (128mm)'},...accessories.filter(a=>a.group==='handle').map(a=>({value:a.acc_id,label:`${a.name} (${useGood?(a.price_good||a.price):a.price})`}))]
    ...
    <div style={{padding:'24px 28px',overflowY:'auto',flex:1}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <Toggle value={useGood} onChange={setUseGood}/>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'minmax(300px,1fr) minmax(300px,1fr)',gap:24,alignItems:'start'}}>
        <Card>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            ...
            <Field label="Height (cm)" k="height_cm"/><Field label="Doors" k="doors_count"/>
            <Field label="Drawers" k="drawers_count"/><Field label="Shelves" k="shelves_count"/>
            <Field label="Partitions" k="partitions_count"/><div/>
            ...
            <Field label="Hinge Type" k="hinge_id" options={hingeOpts}/>
            <Field label="Handle Type" k="handle_id" options={handleOpts}/>
            <div style={{gridColumn:'1/-1'}}><Field label="Has Mirror" k="has_mirror" type="toggle"/></div>
            <div style={{gridColumn:'1/-1',borderTop:`1px solid ${COLORS.border}`,paddingTop:12}}>
              <Field label="Selling Price (optional)" k="selling_price"/>
            </div>
          </div>
          <Btn size="sm" style={{marginTop:16}} onClick={()=>{
            const s={...form,sku_code:`EST-${Date.now().toString(36).toUpperCase()}`}
            setSkus(p=>[...p,s]); toast('Saved to catalog')
          ...
          {cost && !cost.error && <>
            <Card style={{marginBottom:16}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:6}}><Icon name="zap" size={14} color={COLORS.amber}/><span style={{fontSize:11,color:COLORS.amber,fontWeight:600}}>Live</span></div>
              </div>
              {[
                {l:'Material Cost',v:cost.total_material_cost},
                {l:`Edge Banding (${cost.edge_banding.total_m.toFixed(1)} m)`,v:cost.edge_banding.cost},
                {l:'Accessories',v:cost.accessories.total},
                {l:`Overhead (${(cost.overhead_percent*100).toFixed(0)}%)`,v:cost.overhead_amount},
              ].map(r=>(
              ...
              <div style={{display:'flex',justifyContent:'space-between',padding:'12px 0',fontWeight:800,fontSize:17}}>
                <span style={{color:COLORS.text}}>Production Cost</span><span style={{color:COLORS.accent}}>{fmt(cost.production_cost)}</span>
              </div>
            </Card>
            ...
            {form.selling_price>0 && cost.commercial && <Card style={{marginBottom:16}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',fontSize:13}}>
                <span style={{color:COLORS.textDim}}>Net Profit</span><span style={{fontWeight:700,color:mc}}>{fmt(cost.commercial.net_profit)}</span>
              </div>
            </Card>}
            ...
            <Card>
              ...
              {cost.materials_breakdown.map((mb,i)=>{
                const u=(mb.total_area_cm2/(mb.required_sheets*mb.sheet_area_cm2))*100
                const uc=u>80?COLORS.green:u>50?COLORS.amber:COLORS.red
                return <div key={i} style={{marginBottom:i<cost.materials_breakdown.length-1?12:0}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4,fontSize:12}}>
                    <span style={{color:COLORS.textDim}}>{mb.material_name} — {mb.required_sheets} sheet{mb.required_sheets>1?'s':''}</span>
                    <span style={{fontWeight:600,color:uc}}>{u.toFixed(0)}%</span>
                  </div>
                  <div style={{height:6,borderRadius:3,background:COLORS.border}}><div style={{height:'100%',borderRadius:3,width:`${u}%`,background:uc,transition:'width 0.4s'}}/></div>
                </div>
              })}
            </Card>
          </>}
